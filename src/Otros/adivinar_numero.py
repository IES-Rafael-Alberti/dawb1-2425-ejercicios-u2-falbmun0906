...
def configurar_rangos_numeros() -> tuple:
    ...
    valor_correcto_minimo = False
    valor_correcto_maximo = False

    while not valor_correcto:
        while not valor_correcto_minimo:
            try:
                minimo = pedir_numero_usuario("Introduce el valor mínimo del rango: ")
                valor_correcto_minimo = True
            except ValueError as e:
                mostrar_error(e)
                limpiar_pantalla()
                mostrar_titulo(4)

        while not valor_correcto_maximo:
            try:
                maximo = pedir_numero_usuario("Introduce el valor máximo del rango: ")
                valor_correcto_maximo = True
            except ValueError as e:
                mostrar_error(e)
                limpiar_pantalla()
                mostrar_titulo(4)
        ...
        if maximo <= minimo:
            ...
            valor_correcto = True
        valor_correcto_minimo, valor_correcto_maximo = False, False
        mostrar_titulo(4)
...
def configurar_pistas(minimo: int, maximo: int) -> tuple:
    ...
    valor_correcto = False
    valor_correcto_frio = False
    valor_correcto_caliente = False
    maximo = maximo - minimo
    minimo = 1
    while not valor_correcto:
        mostrar_titulo(4)
        while not valor_correcto_frio:
            try:
                frio = pedir_numero_usuario("Introduce la diferencia a partir de la cual se considera FRÍO: ", minimo, maximo)
                valor_correcto_frio = True
            except ValueError as e:
                mostrar_error(e)
                limpiar_pantalla()
                mostrar_titulo(4)

        while not valor_correcto_caliente:
            try:
                caliente = pedir_numero_usuario("Introduce la diferencia a partir de la cual se considera CALIENTE: ", minimo, maximo)
                valor_correcto_caliente = True
            except ValueError as e:
                mostrar_error(e)
                limpiar_pantalla()
                mostrar_titulo(4)
        ...
        elif caliente <= 0 or caliente >= maximo:
            mostrar_error("La diferencia para la pista CALIENTE debe estar entre 0 y el máximo permitido!")
        else:
            valor_correcto = True
        valor_correcto_frio, valor_correcto_caliente = False, False
        limpiar_pantalla()
...
def mostrar_configuracion(minimo, maximo, intentos, frio, caliente, pantalla_inicial = False):
    """
    ...
    print(f"* Pista CALIENTE si la diferencia es menor a {frio} y mayor a {caliente}.")
    print(f"* Pista TE QUEMAS si la diferencia es menor a {caliente}.")
